...
This file provides guidance when working with code in this repository. **Personal OS** (goals, tasks, `Knowledge/`) lives in `AGENTS.md` — use that for productivity context; use this file for the **static site** only.

**Before doing any work for Kyle:** read `Cowork Station/ABOUT ME/` for context on who he is, how he writes, and what he cares about. That folder is the single source of truth.

**Save all deliverables** to `Cowork Station/OUTPUTS/` under a subfolder named after the project. Do not save outputs inside this repo.
